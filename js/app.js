...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <button id="details-btn" onclick='showDetails(${product.price},${product.rating.rate},${product.id})' class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#exampleModal">Details</button>
      </div>
      `;
...
const showDetails = (price, rating, id) => {
  document.getElementById("modal-body").innerHTML = `<div class='p-3'>
    <h4>Id: ${id}</h4>  
    <h4>Price: $ ${price}</h4>
...
const addToCart = (id, price) => {
  ...
  //Call for Update Tax, Total Products & grand Total
  updateTaxAndCharge();
...
document.getElementById("buyBtn").addEventListener("click", function () {
  ...
  confirm(`Please Pay ${total} TK to Confirm Order`);
});
